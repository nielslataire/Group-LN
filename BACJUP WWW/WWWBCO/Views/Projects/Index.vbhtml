                                    ...
                                    @<text>
                                        <span class="thumb-info thumb-info-lighten">
                                            <span class="thumb-info-wrapper m-none">
                                                @If Model.SalesData.Where(Function(m) m.ProjectId = project.Id).FirstOrDefault().PercentageLivingUnitsSold < 15 Or Model.SalesSettings.Where(Function(m) m.ProjectId = project.Id).FirstOrDefault().SaleVisible = False Then
                                                ...
                                                ElseIf Model.SalesData.Where(Function(m) m.ProjectId = project.Id).FirstOrDefault().LivingUnits = 0 Then
                                                    @<text>
                                                ...
                                                <span class="feature-tag background-color-secondary text-md" data-width="40" data-height="50" style="color: rgb(255, 255, 255); text-transform: uppercase; padding: 20px 87px;padding-bottom:10px; position: absolute; right: -25%; top: -1%; transform: rotate(45deg); transition: none; text-align: center; line-height: 15px; border-width: 0px; margin: 0px; letter-spacing: 0px;">
                                                    <span class="font-weight-bold">@String.Format("{0:n0}", Math.Ceiling(Model.SalesData.Where(Function(m) m.ProjectId = project.Id).FirstOrDefault().PercentageLivingUnitsSold / 5) * 5) %</span><br /><span class="text-xs">verkocht</span>  </span>
                                                    </text>
                                                End If
