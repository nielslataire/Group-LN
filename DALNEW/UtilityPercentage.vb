'------------------------------------------------------------------------------
' <auto-generated>
'     This code was generated from a template.
'
'     Manual changes to this file may cause unexpected behavior in your application.
'     Manual changes to this file will be overwritten if the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic

Partial Public Class UtilityPercentage
    Public Property ClientAccountId As Integer
    Public Property IncommingInvoiceDetailId As Nullable(Of Integer)
    Public Property ContractId As Nullable(Of Integer)
    Public Property Percentage As Decimal

    Public Overridable Property ClientAccount As ClientAccount
    Public Overridable Property Contract As Contract
    Public Overridable Property IncommingInvoiceDetail As IncommingInvoiceDetail

End Class
