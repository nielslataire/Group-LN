@modeltype WWWCOPRO.ProjectSendDocModel
...
@Using Html.BeginForm("SendPlanConfirm", "Projects", FormMethod.Post, New With {.id = "FormSendPlan2", .Class = "mb-none"})
    ...
    @<text>
        ...
        <input type="hidden" name="g-recaptcha-response" id="gRecaptchaResponseDoc" />
        <div class="contact-modal-container" id="modalsenddocpanel">
            ...
            <div class="contact-modal-header">
                ...
                <h3 class="contact-modal-title">Document opvragen</h3>
                <div class="contact-modal-deco"></div>
                <p class="contact-modal-subtitle">Het gevraagde document wordt u direct per e-mail toegestuurd.</p>
            </div>
    ...
    $("#FormSendPlan2").submit(function (event) {
        ...
        if (window.reCaptchaSiteKey && typeof grecaptcha !== 'undefined') {
            grecaptcha.ready(function () {
                grecaptcha.execute(window.reCaptchaSiteKey, { action: 'senddoc' }).then(function (token) {
                    $("#gRecaptchaResponseDoc").val(token);
                    doSubmit();
                }, doSubmit);
            });
        } else {
