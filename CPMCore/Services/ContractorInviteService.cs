...
public class ContractorInviteService : IContractorInviteService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var inviteResult = await _guestInviteService.InviteGuestAsync(
            user.Id, invitedByUserId, appBaseUrl, ct, loginPath: "/Werfportaal");
        ...
        CancellationToken ct = default)
    {
        ...
        var result = await _guestInviteService.InviteGuestAsync(userId, invitedByUserId, appBaseUrl, ct, loginPath: "/Werfportaal");
        if (!result.Success)
            _logger.LogWarning(
